...
2. Quick steps / 快速步驟:
   ...
   b. Add an entry below with status="verified" or "inferred".
      在下方加入條目，狀態標 "verified" 或 "inferred"。
...
3. After a successful build, send a PR back to this recipe with status upgraded
   ...
   build 成功後，請發 PR 回本工具把狀態升為 "verified"，讓其他人受惠。
"""
...
LANGUAGES = {
    ...
    "TC": {
        ...
        # SHA256 是本工具作者那份 tar 的 hash；build.sh 預設 CHECK_HASH 為空，不會驗。
        # 若你的 tar 不同，請替換。
        "manifest_entries": {
            "14.5.1": {
